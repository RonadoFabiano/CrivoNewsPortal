...
const TYPE_META: Record<string, { label: string; color: string; bg: string; border: string }> = {
  country: { label: "País",   color: "#1d4ed8", bg: "#eff6ff", border: "#bfdbfe" },
  person:  { label: "Pessoa", color: "#b45309", bg: "#fffbeb", border: "#fed7aa" },
  topic:   { label: "Tema",   color: "#15803d", bg: "#f0fdf4", border: "#bbf7d0" },
};

const RANK_STYLE = [
  { bg: "#fff8e6", color: "#f59e0b", border: "#fde68a", symbol: "①", size: "17px" },
  { bg: "#f8f8fc", color: "#94a3b8", border: "#e2e8f0", symbol: "②", size: "15px" },
  { bg: "#fff7ed", color: "#b87333", border: "#fed7aa", symbol: "③", size: "15px" },
];
...
function TopicoItem({ item, rank, maxCount, onClick }: {
  item: TrendingItem; rank: number; maxCount: number; onClick: () => void;
}) {
  ...
  // Microtexto simulado de variação
  const microVariations = ["+18 nas últimas 6h", "+22 nas últimas 6h", "−5 nas últimas 6h",
    "+4 nas últimas 6h", "+11 nas últimas 6h", "+7 nas últimas 6h", "+3 nas últimas 6h", "+9 nas últimas 6h"];
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px" }}>
          ...
          <span style={{
            ...
            letterSpacing: "0.3px", color: meta.color, background: meta.bg,
...
export default function TopicosDestaque() {
  ...
  useEffect(() => {
    getCachedTrending().then(items =>
      setTopics(
        items.filter(i => i.type === "topic" || i.type === "country" || i.type === "person")
             .slice(0, 8)
      )
    );
  }, []);
